...
//    typeInfo<T>()
    ...
    when (dateType) {
        kotlinx -> newServer<KotlinInstantTask, Instant>(host, port, storageType, engine, json)
        java8 -> newServer<JavaOffsetDateTimeTask, OffsetDateTime>(host, port, storageType, engine, json)
    }.start(wait = true)
}

inline fun <reified T : Task<D>, reified D> newServer(
    host: String, port: Int, storageType: StorageType, engine: EngineType, json: JsonType
) = newServer<T, D>(
